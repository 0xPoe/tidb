...
func (b *PlanBuilder) buildSimple(ctx context.Context, node ast.StmtNode) (base.Plan, error) {
	...
	case *ast.FlushStmt:
		if raw.Tp == ast.FlushStatsDelta {
			...
			b.requireSelectPrivForStatsObjects(raw.FlushObjects, false)
			break
		}
...
func (b *PlanBuilder) buildRefreshStats(rs *ast.RefreshStatsStmt) (base.Plan, error) {
	...
	rs.Dedup()
	b.requireSelectPrivForStatsObjects(rs.RefreshObjects, true)
	p := &Simple{
		Statement:    rs,
		IsFromRemote: false,
...
func (b *PlanBuilder) requireSelectPrivForStatsObjects(objects []*ast.StatsObject, allowRestoreAdmin bool) {
	if len(objects) == 0 {
		intest.Assert(len(objects) > 0, "stats objects should not be empty")
		return
	}

	if allowRestoreAdmin {
