...
// Matches returns true if the context is internal stats processing source.
func (*CtxMatcher) Matches(x any) bool {
	ctx := x.(context.Context)
	s := util.RequestSourceFromCtx(ctx)
	return s == util.InternalRequest+"_"+kv.InternalTxnStatsProcessing
}

// String returns the description of CtxMatcher.
func (*CtxMatcher) String() string {
	return "all txns should be internal stats processing source"
}
